...
#income = pd.read_csv(r'C:\Users\Lenovo 7000\OneDrive\Desktop\Dataspark\income_level.csv')
...
#    
#    merged_df = df.merge(income, how = 'left' , left_on ='agent_home_planningarea', right_on = 'Planning Area Code' )
#    merged_df.dropna(inplace = True)
#    
#    # Extract datetime column
#    merged_df['datetime'] = df.timestamp.apply(lambda x: x[:10]+' ' + x[11:19])
#    merged_df.drop('timestamp',axis = 1,inplace = True)
#    merged_df.to_csv('%s.csv' % name)
...
# match income level
income = pd.read_csv(r'C:\Users\Lenovo 7000\OneDrive\Desktop\Dataspark\income_level.csv')
df_concated = df_concated.merge(income, how = 'left' , left_on ='agent_home_planningarea', right_on = 'Planning Area Code' )
...
df_concated.drop(['timestamp','Planning Area Code','code'],axis = 1,inplace = True)
df_concated.dropna(inplace = True)
df_concated.reset_index(inplace = True, drop = True)

df_concated.to_csv('income.csv')
